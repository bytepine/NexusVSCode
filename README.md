...
| 组件 | 要求 |
|------|------|
| **nexus-vscode** | 与 UE `proxy_config.minProxyVersion` 对齐，建议使用最新版 |
...
| 渠道 | 说明 |
|------|------|
| **扩展商店**（推荐） | 在 VSCode / Cursor / CodeBuddy / Windsurf 的扩展面板搜索 **Nexus MCP** 直接安装（经 [Open VSX](https://open-vsx.org/extension/byteyang/nexus-mcp-vscode)） |
| `nexus-mcp-vscode-<version>.vsix` | [NexusVSCode Releases](https://github.com/bytepine/NexusVSCode/releases) 下载后手动安装 |
...
**方式 A — 扩展商店（推荐）**：在 VSCode / Cursor / CodeBuddy / Windsurf 的扩展面板搜索 **Nexus MCP** 并安装。
...
安装后：**Settings** → 搜索 `nexusMcp` → 将 **Nexus Mcp: Enabled** 设为 `true`（默认关闭；开启后立即启动，默认端口 `6900`）。
...
3. `git commit` → `git tag -a nexus-vscode-vX.Y.Z` → `git push origin HEAD` + `git push origin nexus-vscode-vX.Y.Z`

push tag 后 `.github/workflows/release.yml` 打包 `nexus-mcp-vscode-<ver>.vsix` 并发布 Release。
